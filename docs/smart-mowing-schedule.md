...
When those conditions are met, the automation presses the edge cutting button first. It waits for Worx Cloud to confirm that the mower left the dock, then waits until the mower returns to `docked` or `paused`. After a short settling delay, it starts normal mowing for the calculated runtime. If Home Assistant does not see the mower leave the dock after the first normal mowing command, the automation sends the start command one more time before treating the run as failed. If rain starts during the edge pass, or if the edge pass does not finish before the configured timeout, the automation docks the mower and does not reset the growth helper.
