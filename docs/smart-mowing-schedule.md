...
When those conditions are met, the automation sends a normal one-time mowing command with the calculated runtime. If Home Assistant does not see the mower leave the dock after the first command, the same normal mowing command is sent one more time before treating the run as failed. After the normal mowing time, the mower is sent back to the dock if needed; when it is safe and dry, the automation starts a separate edge-only pass. If rain starts while mowing, the mower returns to dock and the growth estimate is only partially reduced, so the automation can retry later.
